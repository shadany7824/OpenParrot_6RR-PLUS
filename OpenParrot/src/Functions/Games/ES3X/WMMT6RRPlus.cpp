...
static void GenerateDongleData(bool isTerminal)
{
	...
	if (isTerminal)
	{
		memcpy(hasp_buffer + 0xD00, "287811521274", 12); // not sure these are OK, since its from google lol.
		hasp_buffer[0xD3E] = GenerateChecksum(hasp_buffer, 0xD00, 62);
		hasp_buffer[0xD3F] = hasp_buffer[0xD3E] ^ 0xFF;
	}
	else
	{
		memcpy(hasp_buffer + 0xD00, "287813521274", 12);
		hasp_buffer[0xD3E] = GenerateChecksum(hasp_buffer, 0xD00, 62);
		hasp_buffer[0xD3F] = hasp_buffer[0xD3E] ^ 0xFF;
	}
}
...
static __int64 __fastcall nbamUsbFinderGetSerialNumber(int a1, char *a2)
{
	...
	{
		static char *serial = "287811521274";
		memcpy(a2, serial, strlen(serial));
		return 0;
	}
	else
	{
		static char *serial = "287813521274";
		memcpy(a2, serial, strlen(serial));
		return 0;
	}
}
...
static InitFunction Wmmt6RRPFunc([]()
								 {
	prepareCerts();
	...
	// Initialize AMAuth bypass (replaces real ALL.Net auth with local stub)
	std::string serverHostname = config["General"]["ServerHostname"];
	if (serverHostname.empty()) serverHostname = "127.0.0.1";
	initAmauth(serverHostname);
	...
	// Hook dongle funcs
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_write", Hook_hasp_write, NULL);
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_read", Hook_hasp_read, NULL);
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_get_size", Hook_hasp_get_size, NULL);
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_decrypt", Hook_hasp_decrypt, NULL);
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_encrypt", Hook_hasp_encrypt, NULL);
        MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_logout", Hook_hasp_logout, NULL);
	MH_CreateHookApi(L"hasp_windows_x64_29775.dll", "hasp_login", Hook_hasp_login, NULL);
	...
	// RVA 0x772D80 = correct offset for this exe version (timestamp 2023-11-21)
	MH_CreateHook((LPVOID)(imageBase + 0x772D80), Hook_InfoSystemCheck, reinterpret_cast<LPVOID*>(&orig_sub_140732030));
	...
	// Enable all print
	injector::WriteMemory<BYTE>(imageBase + 0xA03463, 0xEB, true);

	// Dongle crap
	injector::WriteMemory<BYTE>(imageBase + 0x76EE88, 0xEB, true);
	injector::WriteMemory<WORD>(imageBase + 0xE03E91, 0xE990, true);

	// Fix dongle error (can be triggered by various USB hubs, dongles
	injector::MakeNOP(imageBase + 0xA0F4FF, 2, true);
	...
	{
		injector::WriteMemory<BYTE>(imageBase + 0xB8590A, 0xEB, true); //content router patch
		injector::MakeNOP(imageBase + 0x7602A6, 2, true);
		...
		// What is this
		injector::MakeNOP(imageBase + 0x77060A, 6, true);
		injector::WriteMemory<BYTE>(imageBase + 0x77061F, 0xEB, true);

		// SafeNet Check
		injector::MakeNOP(imageBase + 0xA0F4FF, 2, true);

		injector::MakeNOP(imageBase + 0xB4237, 5);
	}

	if (isTerminal) {
		//injector::MakeNOP((imageBase + 0x73B905), 5); //todo
		safeJMP(hook::get_pattern("8B 01 0F B6 40 78 C3 CC CC CC CC"), ReturnTrue);
	}

	MH_EnableHook(MH_ALL_HOOKS); }, GameID::WMMT6RRPlus);
#endif
#pragma optimize("", on)
