...
static void PrepareAMAuth() {
	...
	else if (gameVer == "W6P")
	{
		...
		if (forceVersionEnabled == "1" && !forceVersion.empty())
		{
			...
			std::string configText(reinterpret_cast<const char*>(AMConfig_6RRP), sizeof(AMConfig_6RRP));
			finalConfig = UpdateGameVersion(configText, gameVersion1, gameVersion2);
		}
		else
		{
			if (revision != -1) {
				...
				finalConfig = std::string(reinterpret_cast<const char*>(AMConfig_6RRP), sizeof(AMConfig_6RRP));
			}
		}
		fwrite(finalConfig.c_str(), 1, finalConfig.size(), AMConfigWrite);
		fclose(AMConfigWrite);
...
static void dllreg() {
	...
	if (SUCCEEDED(hr)) {
#ifdef DEBUG
		info(true, "iauthdll.dll registered successfully!");
#endif
	}
	else {
